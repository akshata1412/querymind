...
const handleFileUpload = async () => {
  ...
  try {
    ...
    toast.success(`Loaded ${data.table_count} tables with ${data.rows_loaded} total rows!`)
    onConnect(data.schema, data.db_url)
  } catch (e) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: "600px", height: "600px", borderRadius: "50%", background: "rgba(83,221,252,0.04)", filter: "blur(160px)" }} />
      ...
      <style>{`
        ...
        .connect-input:focus { border-color: ${styles.primary} !important; outline: none; }
        .tab-btn:hover { background: rgba(163,166,255,0.06) !important; }
        .example-btn:hover { color: ${styles.primary} !important; }
        .dropzone:hover { border-color: rgba(163,166,255,0.5) !important; }
        .primary-btn:hover:not(:disabled) { transform: scale(1.01); }
            ...
            {activeTab === "upload" && (
              <div>
                <p style={{ color: styles.onSurfaceVariant, fontSize: "13px", marginBottom: "20px", textAlign: "center" }}>
                  Upload a CSV file — we'll analyze it instantly
                </p>
                ...
                >
                  <div style={{ width: "56px", height: "56px", borderRadius: "50%", backgroundColor: styles.surfaceContainerHigh, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                    <Upload color={styles.primaryContainer} size={24} />
                  </div>
                  <p style={{ color: styles.onSurface, fontWeight: 600, margin: "0 0 6px", fontSize: "15px" }}>
                    {dragOver ? "Drop files here!" : "Drop your CSV file here"}
                  </p>
                  <p style={{ color: styles.onSurfaceVariant, fontSize: "12px", margin: "0 0 12px" }}>or click to browse</p>
                  <span style={{ fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.1em", color: styles.onSurfaceVariant, fontWeight: 700, padding: "4px 12px", backgroundColor: styles.surfaceContainer, borderRadius: "999px" }}>CSV only · Max 50MB</span>
                ...
                {selectedFiles.length > 0 && (
                  <div style={{ marginBottom: "16px" }}>
                    <p style={{ color: styles.onSurfaceVariant, fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "8px" }}>
                      Selected File
                    </p>
                    {selectedFiles.map((file, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 14px", backgroundColor: styles.surfaceContainer, borderRadius: "8px", marginBottom: "6px" }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <p style={{ color: styles.onSurfaceVariant, fontSize: "11px", margin: 0 }}>{(file.size / 1024).toFixed(0)} KB · Ready to analyze</p>
                        </div>
                        <button
                          onClick={() => removeFile(i)}
                          style={{ background: "none", border: "none", cursor: "pointer", color: styles.onSurfaceVariant, padding: "4px", borderRadius: "4px", display: "flex" }}
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Zap size={16} />
                      {selectedFiles.length > 0 ? `Analyze ${selectedFiles.length} file(s)` : "Select a File First"}
                    </>
                  )}
                </button>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  style={{
                    ...
                    fontFamily: "monospace", boxSizing: "border-box", transition: "all 0.2s"
                  }}
                ...
                <div style={{ backgroundColor: "#000000", borderRadius: "8px", padding: "12px 16px", marginBottom: "20px", border: `1px solid rgba(64,72,93,0.15)` }}>
                  ...
                  {["sqlite:///./demo.db", "mysql+pymysql://user:pass@host/db", "postgresql://user:pass@host/db"].map(example => (
                    <p key={example} className="example-btn" onClick={() => setUrl(example)}
                ...
                <button
                  ...
                  style={{
                    ...
                    justifyContent: "center", gap: "8px", fontFamily: "Inter, sans-serif", transition: "all 0.2s",
                  }}
                >
                  {loading ? (
                    <>
